...
class AdaptiveSignalPipeline:
    ...
    def _calculate_webster_timing(
        ...
        green_pool = max(cycle_length - self.CONSTANTS.lost_time_sec, len(self.DIRECTIONS) * min_green_sec)
        signal_plan: Dict[str, dict] = {}

        for dir_name in self.DIRECTIONS:
            y_i = y_values[dir_name]
            if y_total > 0:
                allocated = (y_i / y_total) * green_pool
            else:
                allocated = min_green_sec
            green_time = max(min_green_sec, min(allocated, max_green_sec))
