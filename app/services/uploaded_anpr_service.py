...
def _configure_easyocr_ssl() -> None:
    """Use certifi's CA bundle for EasyOCR's first-run model downloads."""
    try:
        import certifi
    except ImportError:
        return
    ...
    def _certifi_context(*args, **kwargs):
        kwargs.setdefault("cafile", ca_file)
        return ssl.create_default_context(*args, **kwargs)

    ssl._create_default_https_context = _certifi_context


def _easyocr_download_error() -> str:
    return (
        "EasyOCR could not download its first-run model files. Check your "
        "internet connection and SSL certificates, then retry the ANPR scan."
    )
...
def _get_anpr_components():
    ...
    try:
        ...
        raise RuntimeError(_easyocr_download_error()) from exc
