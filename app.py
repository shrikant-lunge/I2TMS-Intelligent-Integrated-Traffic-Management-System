...
app.config["SQLALCHEMY_DATABASE_URI"] = (
    "sqlite:///" + os.path.join(BASE_DIR, "database", "i2tms.db")
)
...
class EmergencyRequest(db.Model):
    ...
    vehicle_type = db.Column(db.String, nullable=False)          # 'ambulance' | 'fire_truck'
    ...
    priority = db.Column(db.String, default='high')
...
def api_junction_signal():
    ...
    cycle_sec    = 90
    elapsed      = int(_time.time()) % cycle_sec
...
@app.route("/api/hold_current", methods=["POST"])
@login_required
...
@app.route("/api/junction_analytics")
@login_required
...
def create_emergency_request():
    ...
    # --- REAL PIPELINE PLACEHOLDER: replace with real OSRM/GraphHopper routing call ---
    distance_km = round(random.uniform(3, 12), 1)
    eta_min = round(distance_km * 2)
    signals_on_route = random.randint(3, 8)
    vms_on_route = random.randint(1, 5)
    # --- end replace block ---

    # Generate simulated geometry (origin, intermediate point to make a nice route curve, and destination)
    c_lat = float(body.get("current_lat") or 21.1458)
    c_lng = float(body.get("current_lng") or 79.0526) # Nagpur center fallback
    d_lat = float(body.get("destination_lat") or 21.1428)
    d_lng = float(body.get("destination_lng") or 79.0780)

    # Let's generate a mock route polyline bend to look realistic
    mock_geometry = [
        [c_lat, c_lng],
        [c_lat + (d_lat - c_lat) * 0.4 + 0.002, c_lng + (d_lng - c_lng) * 0.4 - 0.002],
        [c_lat + (d_lat - c_lat) * 0.8 - 0.001, c_lng + (d_lng - c_lng) * 0.8 + 0.001],
        [d_lat, d_lng]
    ]
...
@app.route("/api/emergency_request/<int:req_id>")
@login_required
...
@app.route("/api/emergency_request/<int:req_id>/end", methods=["POST"])
@login_required
...
@app.route("/emergency/route/<int:request_id>")
@login_required
...
@app.route("/api/incidents_alerts")
@login_required
...
@app.route("/api/live_video_stats")
@login_required
...
@app.route("/api/junctions", methods=["GET", "POST"])
@login_required
...
@app.route("/api/junctions/<int:j_id>", methods=["PUT"])
@login_required
...
@app.route("/api/vms_boards", methods=["GET", "POST"])
@login_required
...
@app.route("/api/vms_boards/<int:v_id>", methods=["PUT"])
@login_required
...
@app.route("/api/users/<int:u_id>/reset_password", methods=["POST"])
@login_required
...
@app.route("/api/users/<int:u_id>/deactivate", methods=["POST"])
@login_required
